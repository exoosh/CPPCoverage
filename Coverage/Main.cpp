...
int main(int argc, const char** argv)
{
    ...
    try
    {
        ...
        else
        {
            ...
            // Grab any auxiliary (sibling-bitness) coverage outputs the runner
            // produced so we can fold them into the merge step below.
    ...
    try
    {
        if (opts.ConsolidateAuxiliary && !auxiliaryOutputs.empty())
        {
            ...
            else
            {
                ...
                // Everything has been folded in; don't let the -m step
                // below try to re-merge the (now-deleted) aux files.
    ...
    try
    {
        if(!opts.MergedOutput.empty())
        {
            ...
            // processes (see CoverageRunner for how these are produced).
            // When -consolidate was used, auxiliaryOutputs is empty here
            // because everything has already been folded into our local
